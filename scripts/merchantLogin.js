...
document.addEventListener("DOMContentLoaded", () => {
  ...
  if (!token) {
    console.warn("Missing token in sessionStorage, redirecting to signup"); // Debug
    if (usernameError) {
      usernameError.textContent =
        "Authentication required. Please sign up or try again.";
    }
    setTimeout(() => {
      ...
      try {
        ...
        if (error.message.includes("token")) {
          showError(
            usernameInput,
            usernameError,
            "Authentication failed. Please sign up or try again."
          );
          setTimeout(() => {
            window.location.href = "../sign-up/";
          }, 2000);
